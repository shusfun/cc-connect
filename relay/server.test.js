...
test("push routes do not exist", async () => {
...
test("clientAddressLabel is stable for one secret and never exposes the address", () => {
  ...
  const first = clientAddressLabel(request, { trustProxy: true, secret });
  const second = clientAddressLabel(request, { trustProxy: true, secret });

  assert.equal(first, second);
  assert.match(first, /^client#[0-9a-f]{12}$/);
...
test("relay logs redact live session identifiers", async () => {
  ...
  assert.ok(capturedLogs.some((line) => line.includes("remote=client#")));
  assert.ok(capturedLogs.every((line) => !line.includes("session-sensitive")));
  assert.ok(capturedLogs.every((line) => !line.includes("127.0.0.1")));
});
