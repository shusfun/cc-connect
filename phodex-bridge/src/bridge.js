...
const { createSyncCoordinator } = require("./sync-journal");
...
function startBridge({
  ...
  codex.onMessage((message) => {
    ...
    syncCoordinator.observeCodexMessage(message, parsedMessage);
  ...
  function handleApplicationMessage(rawMessage) {
    ...
    if (syncCoordinator.handleRequest(rawMessage, sendApplicationResponse, parsedMessage)) {
      return;
    }
  ...
  function handleBridgeManagedHandshakeMessage(rawMessage, sendResponse = sendApplicationResponse, parsedMessage = null) {
    ...
    if (method === "initialize" && parsed.id != null) {
      rememberInitializingClientDevice(parsed.params || {});
  ...
  // Retains only non-sensitive device-kind metadata for connection diagnostics.
  function rememberInitializingClientDevice(params) {
