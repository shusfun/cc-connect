...
final class CodexServiceCatchupRecoveryTests: XCTestCase {
    ...
    func testTurnStateReadDoesNotFallbackWhenPaginationIsUnsupported() async {
        let service = makeService()
        let threadID = "thread-unsupported-pagination"
        ...
        do {
            _ = try await service.readThreadTurnStateSnapshot(threadId: threadID)
            XCTFail("Expected unsupported pagination to fail")
        } catch {
