// FILE: VoiceRecordingManager.swift
// Purpose: Captures microphone audio with AVAudioEngine and produces normalized 24 kHz mono WAV clips for local transcription.
// Layer: Service
// Exports: VoiceRecordingClip, VoiceRecordingManager
...
struct VoiceRecordingClip: Sendable {
    let url: URL
    let mimeType: String
...
final class VoiceRecordingManager: ObservableObject {
    ...
    func startRecording() async throws {
        ...
        do {
            ...
            guard format.sampleRate > 0, format.channelCount > 0 else {
                ...
                throw VoiceRecordingError.missingMicrophoneInput
            }
            ...
            throw VoiceRecordingError.unableToPrepareAudioEngine
        }
    }

    // Stops the capture, resamples collected audio to 24 kHz mono, and returns the clip.
    @MainActor
    func stopRecording(preferM4A: Bool = false) throws -> VoiceRecordingClip? {
    ...
    private func configureAudioSession() async throws {
        do {
            ...
            throw VoiceRecordingError.unableToConfigureAudioSession
        }
    }
    ...
    private static func writeWAVFile(samples: [Int16]) throws -> (url: URL, mimeType: String, byteCount: Int) {
        ...
        do {
            ...
            throw VoiceRecordingError.unableToCreateOutputFile
        }
    }
...
private final class MeterWindowAccumulator: @unchecked Sendable {
    ...
    func beginSession(sampleRate: Double) {
        lock.lock()
        windowSampleCount = max(1, Int(sampleRate * VoiceRecordingManager.meterWindowSeconds))
        sumOfSquares = 0
        sampleCount = 0
        lock.unlock()
    }
