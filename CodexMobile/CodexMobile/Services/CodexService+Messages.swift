...
extension CodexService {
    ...
    // Loads the current private-sync delta once per thread. Older pages remain behind the reset cursor.
    @discardableResult
    func loadThreadHistoryIfNeeded(
        ...
        if messagesByThread[threadId] == nil {
            let cachedMessages = messagePersistence.load(
                threadId: threadId,
            ...
            if !cachedMessages.isEmpty {
                messagesByThread[threadId] = cachedMessages
                noteMessagesChanged(for: threadId)
                refreshThreadTimelineState(for: threadId)
            }
        }
        let syncState = CodexDerivedCache.shared.syncState(
            macDeviceId: currentMacScopedPersistenceDeviceId,
            scope: "thread:\(threadId)"
        )
        let changed = try await syncPrivateThread(
            threadId: threadId,
            forceReset: !syncState.initialized
        )
        hydratedThreadIDs.insert(threadId)
        initialTurnsLoadedByThreadID.insert(threadId)
        refreshThreadTimelineState(for: threadId)
        return changed ? .loadedPaginatedWindow : .alreadyHydrated
    }
