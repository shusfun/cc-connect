...
extension CodexService {
    ...
    func scheduleCurrentMacThreadListSnapshotPersistence() {
        ...
        let capturedPinnedThreadIDs = Set(pinnedThreadIDs)
        let capturedActiveThreadID = activeThreadId
        ...
        threadListSnapshotPersistenceTask = Task { @MainActor [weak self] in
            ...
            self.threadListPersistence.save(
                snapshot,
                macDeviceId: capturedMacDeviceId,
                pinnedThreadIDs: capturedPinnedThreadIDs,
                activeThreadID: capturedActiveThreadID
            )
        }
    }
...
private extension CodexService {
    ...
    func mergeMacScopedMessages(from sourceDeviceIds: [String], to targetDeviceId: String) -> Bool {
        var targetMessages = messagePersistence.loadAll(macDeviceId: targetDeviceId)
        var changed = false

        for sourceDeviceId in sourceDeviceIds {
            defer { messagePersistence.delete(macDeviceId: sourceDeviceId) }
            let sourceMessages = messagePersistence.loadAll(macDeviceId: sourceDeviceId)
