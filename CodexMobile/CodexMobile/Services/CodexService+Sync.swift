...
extension CodexService {
    ...
    func syncThreadHistory(threadId: String, force: Bool = false) async {
        ...
        do {
            let requiresInitialReset = !CodexDerivedCache.shared
                .syncState(
                    macDeviceId: currentMacScopedPersistenceDeviceId,
                    scope: "thread:\(threadId)"
                )
                .initialized
            _ = try await syncPrivateThread(
                threadId: threadId,
                forceReset: force && requiresInitialReset
            )
        } catch {
    ...
    func removeThreadLocally(_ threadId: String, persistAsDeleted: Bool, persistMessages: Bool = true) {
        if let turnId = activeTurnID(for: threadId) {
            setActiveTurnID(nil, for: threadId)
            threadIdByTurnID.removeValue(forKey: turnId)
            if activeTurnId == turnId { activeTurnId = nil }
    ...
    // Pulls only journal events for the visible thread; a journal gap resets this thread alone.
