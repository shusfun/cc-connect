...
final class BridgeMenuBarStore: ObservableObject {
    ...
    private var managementRefreshTask: Task<Void, Never>?
    private var wakeObserver: NSObjectProtocol?
    private var manualRefreshTask: Task<Void, Never>?
    private var observedRelayError: (code: String, message: String)?
    ...
    init() {
        ...
        wakeObserver = NSWorkspace.shared.notificationCenter.addObserver(forName: NSWorkspace.didWakeNotification, object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor in self?.service.didWake() }
        }
        refreshLoopTask = Task { @MainActor [weak self] in
            ...
            while !Task.isCancelled {
                ...
                if let self { self.service.recoverIfNeeded(snapshot: self.snapshot) }
            }
        }
        managementRefreshTask = Task { @MainActor in
            while !Task.isCancelled {
    ...
    private func refreshSnapshot() async {
        let next = await service.loadSnapshot(relayOverride: relayOverride)
        guard !Task.isCancelled else { return }
        snapshot = next
        logUnavailable = service.logFailure
        if next.bridgeStatus?.connectionStatus == "connected", let observedRelayError {
            if errorCode == observedRelayError.code && errorMessage == observedRelayError.message {
                errorCode = ""
                errorMessage = ""
            }
            self.observedRelayError = nil
        }
        if !service.recoveryPhase.isEmpty { phase = service.recoveryPhase; return }
    ...
    func stopBridge() {
        runAction(successMessage: "Bridge 已停止。") {
            ...
            guard !self.service.isRunning else { throw BridgeRuntimeError.commandFailed("停止未完成，请查看诊断。") }
            await self.refreshSnapshot()
        }
    }
    ...
    func copyDiagnostics() {
        Task { @MainActor in
            let text = await service.redactedDiagnostics(relayOverride: relayOverride)
    ...
    private func startOnLaunch() async {
        guard DeviceAccessService.shared.isActivated else { await refreshSnapshot(); return }
        isRefreshing = true
    ...
    private func waitForPairing() async throws {
        ...
        for _ in 0..<150 {
            ...
            await refreshSnapshot()
            guard service.generation == generation else { throw CancellationError() }
            ...
            if let code = snapshot?.bridgeStatus?.lastError, !code.isEmpty {
                ...
                errorCode = code
                observedRelayError = (code, errorMessage)
                if snapshot?.bridgeStatus?.state == "blocked" { throw BridgeRuntimeError.commandFailed(errorMessage) }
            }
        ...
        phase = "正在恢复"
        transientMessage = "连接暂未完成，后台将继续恢复。"
    }
